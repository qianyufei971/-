...
public class PersonalBasicInformationQuery extends JPanel implements ActionListener{
	
	JLabel label1,label2,label3,label4,label5;
	JLabel label101,label201,label301,label401,label501;
	JButton button1,button2;
	String number;
	
	public PersonalBasicInformationQuery(String number) {
		this.number=number;
		label1=new JLabel("编号:");
		label2=new JLabel("姓名:");
		label3=new JLabel("性别:");
		label4=new JLabel("备注:");
		...
		String sql="select * from 系统管理员 where 编号='"+number+"'";
		...
		hBox6.add(button1); hBox6.add(Box.createHorizontalStrut(50));hBox6.add(button2);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==button1) {
			Connection con=GetDBConnect.connectDB("学生宿舍管理系统","root","123456");//连接代码
			String sql="select * from 系统管理员 where 编号='"+this.number+"'";
