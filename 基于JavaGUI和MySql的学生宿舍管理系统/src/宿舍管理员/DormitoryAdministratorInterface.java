...
public class DormitoryAdministratorInterface extends JFrame implements TreeSelectionListener{
	LeftJPanelofDormitoryAdministrator leftJPanel;//左边导航栏
	...
	DormitoryAdministerChangePassword dormitoryAdministerChangePassword;
	Welcome welcome;
	...
	PersonalInformationQuery personalInformationQuery;
	CardLayout cardLayout;
	
	
	public DormitoryAdministratorInterface(String number) {
		 ...
		 try {
			...
			leftJPanel=new LeftJPanelofDormitoryAdministrator();
			rightJPanel=new JPanel();
			welcome=new Welcome();
			aboutTheSystem=new AboutTheSystem();
			studentManagementInterface=new StudentManagementInterface(number);
			dormitoryManagementInterface=new DormitoryManagementInterface(number);
			disciplineViolationInformationManagementInterface=new DisciplineViolationInformationManagementInterface(number);
			visitingInformationManagementInterface=new VisitingInformationManagementInterface(number);
			dormitoryAdministerChangePassword=new DormitoryAdministerChangePassword(number);
			tipsAndTricks=new TipsAndTricks(); 
			help=new Help();
			checkForUpdates=new CheckForUpdates();
			personalInformationQuery=new PersonalInformationQuery(number);
			...
			rightJPanel.add("changepassword",dormitoryAdministerChangePassword);
			rightJPanel.add("tips",tipsAndTricks);
			rightJPanel.add("help",help);
			rightJPanel.add("update",checkForUpdates);
			rightJPanel.add("personalinformation",personalInformationQuery);
