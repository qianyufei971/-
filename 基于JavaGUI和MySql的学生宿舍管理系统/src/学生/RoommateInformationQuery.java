...
public class RoommateInformationQuery extends JPanel implements ActionListener{
	...
	public RoommateInformationQuery(String number) {
		...
		try {
			...
			rs.last();
			louhao=rs.getString(4);
			sushehao=rs.getString(5);
		}catch(Exception e)
		{System.out.println(e);}
		
		
		String sqlsheyou="select * from 学生 where 楼号='"
        		+louhao+"'"+" and 宿舍号='"
        				+ sushehao+"'";
		Statement statementsheyou=null;
		ResultSet rssheyou=null;
		try {
			statementsheyou=con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_READ_ONLY);
			rssheyou=statement.executeQuery(sqlsheyou);
			ResultSetMetaData metaData=(ResultSetMetaData) rssheyou.getMetaData();
			...
			rssheyou.last();
			int recordcount =rssheyou.getRow();
			record=new String[recordcount][columncount];
			rssheyou.beforeFirst();
			int m=0;
			while(rssheyou.next())
			{
				for(int j=1;j<=columncount;j++)
				{
					record[m][j-1]=rssheyou.getString(j);
				}
				m++;
			}
		}catch(Exception e)
